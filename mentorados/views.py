...
def mentorados(request):
  if request.method == 'GET':
    ...
    for i, j in Mentorados.estagio_choices:
      qtds = Mentorados.objects.filter(estagio=i).filter(user=request.user).count()
      qtd_estagios.append(qtds)
    ...
    nome = request.POST.get('nome') # names do input (nome, foto, estagio, navigator)
...
def reunioes(request):
  if request.method == 'GET':
    ...
    disponibilidades = DisponibilidadeHorarios(
      data_inicial = data,
      mentor = request.user,
      # Agendado é igual a 'false' por padrão.
    )
    disponibilidades.save()
    messages.add_message(request, constants.SUCCESS, 'Horário disponibilizado.')
    return redirect('reunioes')
...
def escolher_dia(request):
  ...
  if request.method == 'GET':
    ...
    for i in disponibilidades:
      data_str = i.date().strftime('%d-%m-%Y')
      if data_str not in datas_formatadas:
        dia_semana = i.strftime('%A').title()
        mes = i.strftime('%B').title()
        datas_formatadas[data_str] = {'dia_semana': dia_semana, 'mes': mes}



  return render(request, 'escolher_dia.html', {'datas_info': datas_formatadas})
